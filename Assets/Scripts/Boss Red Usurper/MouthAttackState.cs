...
public class MouthAttackState : StateMachineBehaviour
{   
    GameObject player;
    GameObject monster;
    float preTime =.4f;
    float damageTime = .5f;
    float animationTime = 1.167f;
    float startDamage;
    float endDamage;
    float endAnimation;
    ...
    {   
        ...
        {
            monster.GetComponentInParent<Boss>().MouthAttack();
        }
        else if (Time.time > endDamage && Time.time < endAnimation)
        {
            monster.GetComponentInParent<Boss>().StopMouthAttack();
        } 
